...
public class FoodItemListAdminServlet extends HttpServlet {
	public void init() {
		...
		entries_food
				.add(new FoodItemEntry(entries_food.size(), "Cram Chawder", "Hot Cram Chawder with bacon and cheese ",
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (FoodItemEntry entry : entries_food) {
			...
			out.println("<tbody>" + "<tr>" + "<td>" + entry.getId() + "</td>" + "<td>" + entry.getName() + "</td>"
					...
					+ "<td><a href='foods/delete?id=" + entry.getId() + "'>Delete</a>" + "</td>" + "</tr>"
					+ "</tbody>");
		}
		out.println("</table>");
